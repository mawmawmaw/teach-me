...
A [Claude Code](https://docs.claude.com/en/docs/claude-code) **agent skill** that turns Claude into a personal tutor. Give it a topic and it builds a bespoke, multi-lesson course — calibrated to what you already know — then teaches it lesson by lesson with quizzes, exams, and hands-on assignments, tracking your progress so you can stop and resume anytime.
...
- **Active learning** — a free-form Q&A phase before each quiz, per-lesson multiple-choice + reflection quizzes, and per-module exams with three parts: a written exam, an inline practical scenario, and a take-home assignment that Claude reviews.
...
Or, if you keep skills elsewhere, clone anywhere and symlink it:

```sh
git clone https://github.com/mawmawmaw/teach-me.git
ln -s "$PWD/teach-me" ~/.claude/skills/teach-me
```

Claude Code discovers it automatically on the next session. (The skill sets `disable-model-invocation: true`, so it only runs when you ask for it — see below.)

## Usage

Start a session in the directory where you want the course saved, then ask for it explicitly:

```
/teach-me
```

or just tell Claude what you want to learn, e.g. *"teach me Clojure macros."* It will:
...
To **resume**, start Claude in the same directory and invoke the skill again; it finds the existing `course-*/` folder and picks up where you stopped.
...
HTML:  pandoc --standalone
        ...
        → agent-authored HTML (Claude writes it directly — needs no tooling at all)
```
...
- [`SKILL.md`](SKILL.md) — the skill instructions Claude follows (the workflow).
